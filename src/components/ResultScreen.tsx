...
import { ConfettiBurst, useCountUp } from './motion'
...
export default function ResultScreen({ score, total, answers, isDaily, setLabel, streak, onHome, onRetry, onSaveShare }: Props) {
  const pct = Math.round((score / total) * 100)
  const shownScore = useCountUp(score, 650)
  ...
  return (
    <div className="min-h-screen flex flex-col items-center px-5 py-8"
      ...
      <div className="w-full max-w-sm flex flex-col gap-4">
        ...
        {/* Score summary */}
        <div className="rounded-2xl p-5 text-center relative animate-slide-up"
          style={{ background: '#2D1F52', border: '1px solid #8B6CFF44', boxShadow: '0 0 32px #8B6CFF22' }}>
          {pct >= 80 && <ConfettiBurst spread={140} count={26} />}
          <div className="text-5xl mb-2">{emoji}</div>
          <div className="text-6xl font-black mb-1" style={{ color: '#F5F3FF' }}>{shownScore}/{total}</div>
          ...
          {isDaily && streak !== undefined && streak > 0 && (
            <div className="streak-live inline-flex items-center gap-2 px-4 py-2 rounded-full mb-3"
              style={{ background: '#FBBF2418', border: '1px solid #FBBF2455', ['--glow' as string]: '#FBBF2455' }}>
              <span className="flame text-base">🔥</span>
              <span className="font-black text-lg" style={{ color: '#FBBF24' }}>{streak}</span>
              <span className="text-xs font-semibold" style={{ color: '#FBBF24' }}>day streak</span>
            </div>
          )}
