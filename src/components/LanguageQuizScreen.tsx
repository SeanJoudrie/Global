...
function getChoices(target: LanguageRecord, all: LanguageRecord[], seed: string): LanguageRecord[] {
  ...
  // Prefer confusable languages as distractors
  ...
  const rest = all
    ...
    .sort(() => rng() - 0.5)
    .slice(0, 3 - confusable.length)

  const distractors = [...confusable, ...rest].slice(0, 3)
  const choices = [target, ...distractors].sort(() => rng() - 0.5)
  return choices
}
...
export default function LanguageQuizScreen({ onBack }: Props) {
  ...
  const startQuiz = (diff: Difficulty) => {
    ...
    const qs = shuffled.map((t, i) => ({
      target: t,
      choices: getChoices(t, pool, seed + i),
    }))
    setQuestions(qs)
    setIdx(0)
    setAnswers([])
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-5 gap-5" style={{ zIndex: 1, position: 'relative' }}>
          ...
          <div className="text-center">
            ...
            <p className="text-sm" style={{ color: '#B8A9E0' }}>
              Read a sentence and identify the language from 4 choices.
            </p>
          </div>
      ...
      <div className="flex-1 flex flex-col px-5 py-4" style={{ zIndex: 1, position: 'relative' }}>
          ...
          {q.choices.map((choice, i) => {
            ...
            return (
              <button key={choice.code} onClick={() => handleAnswer(i)} disabled={answered}
                ...
                <div>{choice.name}</div>
                <div className="text-xs font-normal mt-0.5 opacity-70">{choice.nativeName}</div>
              </button>
            )
          })}
        </div>
        ...
        {answered && (
          <div className="rounded-xl p-4 mb-4 animate-slide-up"
            ...
            <p className="text-sm" style={{ color: '#F5F3FF' }}>{q.target.nativeName} is spoken by hundreds of millions worldwide.</p>
          </div>
        )}
