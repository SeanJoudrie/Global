...
function getDistractors(target: FlagRecord, count: number, rngSeed: string): FlagRecord[] {
  ...
  // 2. fill remainder from same region
  ...
  // 3. final fallback from all flags
  if (picked.length < count) {
...
export function buildSetQuiz(flags: FlagRecord[], seed: string): Question[] {
  const shuffled = shuffleWithSeed(flags, seed)
  return shuffled.map(flag => buildQuestion(flag, seed))
}
...
export function generateShareText(result: { score: number; total: number; answers: ('correct' | 'wrong')[]; date: string }): string {
  ...
  return `Daily Globe ${result.date}\n${result.score}/${result.total} 🌍\n${emojiRow}${trickNote}\nPlay at dailyglobe.app`
}
